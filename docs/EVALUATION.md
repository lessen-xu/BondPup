...
封版前复测(2026-08-10,同一生产域名):

| 指标 | P95 |
|---|---|
| MCP `tools/list` | 1.764s |
| MCP 五调用闭环 | 3.562s |
| `/api/agent` 问候(greet) | 1.913s |
| 首访传输量 `/` | 1.16MB(移动端) |
| 示例页传输量 `/?demo=1` | 2.87MB(移动端) |

超时预算链:服务端每个任务共享 9s 总 deadline;首发最多 8s,只有输出闸失败且剩余至少 1.2s 才在同一 deadline 内重试;客户端 10s——网页放弃时服务端必已结束,无白计费。

## 质量闸门

- 152 个单元测试(契约/域计算/安全层/API 契约/注入契约/超时与成本护栏),`tsc --noEmit`,`next build`,全部进 CI;约定红灯不合并(过程中曾有一次红灯误合并,当日发现当日修复,详见 PR #42/#43)
