...
首访传输量(2026-08-10,生产环境 `4db9708`,冷缓存,移动端视口 390×844,只计本站同源请求):

| 页面 | 传输量 | 请求数 | 说明 |
|---|---|---|---|
| `/` | 1.71MB | 15 | 主字体 352KB;回落字体未触发下载 |
| `/?demo=1` | 4.65MB | 20 | 含回落字体 1.35MB 与四张插画(共 1.9MB) |

**已知代价**:回落字体 `manman-hand.woff2` 是 GB2312 一级字库全量子集(1.35MB),示例页出现 Long Cang 缺字时会整包拉取。按实际缺失的 77 字重新子集化可降到约 20KB,效果不变——归复赛优化,不在封版范围内。测量方法与脚本口径见本节表头,复现时请同样排除浏览器扩展注入的请求(实测某拦截类扩展会额外注入 2.27MB,足以让数字失真三倍)。
