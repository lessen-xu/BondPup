...
- 永远不说:超支、赤字、超标、不够、没存够、太多了、太少了、应该、必须、你最好、比上次好、你真棒、加油、建议你、月光族。
      ...
      return {
        instruction:
          input.scene === "decision"
            ? "用户在犹豫要不要买。先接住情绪;若 comfortAvailableText 有值,用『我这里记的安心罐还有 comfortAvailableText』的口吻提一句,金额原样引用,不要自己计算。最后平等给出三个选择:现在买、放到明天、这次先不买。直接输出回应文本,不要 JSON。"
            : input.scene === "note"
